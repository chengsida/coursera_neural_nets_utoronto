...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  z1=data.inputs;
  z2 = model.input_to_hid *z1;
  h=logistic(z2);
  z3 = model.hid_to_class * h;
  norm=log_sum_exp_over_rows(z3);
 log_class_prob = z3 - repmat(norm, [size(z3, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
  y = exp(log_class_prob);
  
  
  ret.input_to_hid =(1/size(data.targets,2)).*model.hid_to_class'*(y-data.targets).* (h.*(ones(size(h))-h))*z1'   +model.input_to_hid .* wd_coefficient;
  ret.hid_to_class =(1/size(data.targets,2)).* (y-data.targets)*h'  + model.hid_to_class .* wd_coefficient;
